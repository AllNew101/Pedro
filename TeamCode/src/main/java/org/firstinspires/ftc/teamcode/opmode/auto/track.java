...
public class track extends OpMode {
    ...
    List data_return = Arrays.asList(0,0,0.0,0.0,0.0);
    ...
    public List track(boolean check_verticle){
        ...
        data_return = Arrays.asList(0,0,0.0,0.0,0,0.0);
        ...
        while ((System.currentTimeMillis() - time2) <= ti_me) {
            ...
            for (ColorBlobLocatorProcessor.Blob myBlob_item : myBlobs) {
                ...
                if (check_verticle == vertical){
                    data_right.add(JavaUtil.createListWith(Math.round(Math.sqrt(Math.pow(Perfect_X - myBoxFit.center.x, 2) + Math.pow(Perfect_Y - myBoxFit.center.y, 2))), vertical, myBoxFit.center.x, myBoxFit.center.y,myBlob_item.getContourArea()));
                }
